...
- The approved visual source is preserved in `web/design-comparison-v3.png`.
- Preserve its calm, conversational Spanish-first home screen, warm white surface, navy typography, indigo actions, green reassurance, compact daily comparison, and five-tab mobile navigation.
- The user prefers low-friction personal capture over dense accounting UI on the first screen.
- At desktop widths the same product must become an administrator/auditor workspace rather than a scaled phone screen: persistent sidebar, filters, reconciled KPIs, analytical charts, detail tables, import review, audit trail, and security status.
